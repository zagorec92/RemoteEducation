	...
	public partial class Login : WindowBase
	{
		...
		private void Button_Click(object sender, RoutedEventArgs e)
		{
			...
			sender.ExecuteIfNotNull<Button>(bttn =>
			{
				...
				if (commandParameter == ApplicationManager.CommandTags.Clear)
					Username = pbxPassword.Password = String.Empty;
				else if (commandParameter == ApplicationManager.CommandTags.Login)
					AuthenticateUser();
				else if (commandParameter == ApplicationManager.CommandTags.Cancel)
					SetWindowTitle(WindowTitles.Login);
				else if (commandParameter == ApplicationManager.CommandTags.Recover)
					ResetPassword();
			});
		}
		...
		private async void AuthenticateUser()
		{
			try
			{
				...
				//loading screen test
				//await Task.Delay(EnumCollection.SleepTime.MS1000.GetValue());
				...
				if (roleID == EnumCollection.RoleType.Admin.GetValue() || roleID == EnumCollection.RoleType.Professor.GetValue())
			...
			catch (ArgumentException ex)
			{
				...
				if (ex.ParamName == AuthenticationManager.AuthenticationFailedParameters.Username)
					UsernameValidationMessage = message;
				else if (ex.ParamName == AuthenticationManager.AuthenticationFailedParameters.Password)
					PasswordValidationMessage = message;
				else if (ex.ParamName == AuthenticationManager.AuthenticationFailedParameters.Parameters ||
					ex.ParamName == AuthenticationManager.AuthenticationFailedParameters.Database)
		...
		private async void ResetPassword()
		{
			...
			await Task.Delay(EnumCollection.SleepTime.MS2500.GetValue());
			UpdateLoadingScreen(isVisible: false);
		}
